...
import { TrendingUp, DollarSign, Calendar, Percent, PiggyBank, Sparkles, ArrowRight } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, Area, AreaChart, TooltipProps } from 'recharts';
...
export default function SIPCalculator(): React.JSX.Element {
  ...
  const calculateSIP = (): void => {
    ...
    const M: number = P * (((Math.pow(1 + r, n) - 1) / r) * (1 + r));
    const totalInvestment: number = P * n;
  ...
  const generateChartData = (monthlyRate: number): void => {
    ...
    for (let year = 0; year <= years; year++) {
      ...
      if (monthlyRate > 0 && months > 0) {
        value = P * (((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate) * (1 + monthlyRate));
      }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-gray-900 text-white p-3 rounded-lg shadow-xl border border-gray-700">
          ...
          <p className="text-blue-400 text-sm">Invested: {formatCurrency(payload[0].payload.invested)}</p>
          <p className="text-green-400 text-sm">Value: {formatCurrency(payload[0].payload.value)}</p>
        </div>
      );
    }
    return null;
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white backdrop-blur-xl rounded-2xl p-6 border border-gray-200 shadow-xl hover:shadow-2xl transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <label className="text-gray-900 font-bold text-lg">Monthly Investment</label>
                  <p className="text-gray-600 text-xs">How much you'll invest each month</p>
                </div>
              </div>
              <div className="space-y-4">
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleMonthlyInvestmentChange(e.target.value)}
                  className="w-full h-3 rounded-lg appearance-none cursor-pointer"
                  style={{
                    background: getSliderBackground(monthlyInvestment, 500, 100000, '#3b82f6', '#8b5cf6', '#e5e7eb')
                  }}
                />
                <input
                  type="number"
            ...
            <div className="bg-white backdrop-blur-xl rounded-2xl p-6 border border-gray-200 shadow-xl hover:shadow-2xl transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <label className="text-gray-900 font-bold text-lg">Expected Return</label>
                  <p className="text-gray-600 text-xs">Annual growth rate (% p.a.)</p>
                </div>
              </div>
              <div className="space-y-4">
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleExpectedReturnChange(e.target.value)}
                  className="w-full h-3 rounded-lg appearance-none cursor-pointer"
                  style={{
                    background: getSliderBackground(expectedReturn, 1, 30, '#10b981', '#34d399', '#e5e7eb')
                  }}
                />
                <input
                  type="number"
            ...
            <div className="bg-white backdrop-blur-xl rounded-2xl p-6 border border-gray-200 shadow-xl hover:shadow-2xl transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <label className="text-gray-900 font-bold text-lg">Time Period</label>
                  <p className="text-gray-600 text-xs">Investment duration in years</p>
                </div>
              </div>
              <div className="space-y-4">
                <input
                  ...
                  step="1"
                  value={timePeriod}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleTimePeriodChange(e.target.value)}
                  className="w-full h-3 rounded-lg appearance-none cursor-pointer"
                  style={{
                    background: getSliderBackground(timePeriod, 1, 40, '#8b5cf6', '#a78bfa', '#e5e7eb')
                  }}
                />
                <input
                  type="number"
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="grid sm:grid-cols-3 gap-4">
              <div 
                className={`bg-gradient-to-br from-blue-500/20 to-blue-600/20 backdrop-blur-xl rounded-2xl p-6 border border-blue-400/30 shadow-2xl transform transition-all duration-300 cursor-pointer ${activeCard === 'invested' ? 'scale-105 ring-4 ring-blue-400' : 'hover:scale-105'}`}
                onMouseEnter={() => setActiveCard('invested')}
                onMouseLeave={() => setActiveCard(null)}
              >
                <div className="flex items-center gap-2 mb-3">
                  <PiggyBank size={20} className="text-blue-600" />
                  <p className="text-blue-700 text-sm font-semibold">Invested Amount</p>
                </div>
                <p className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{formatCurrency(results.totalInvestment)}</p>
                <div className="h-1 bg-blue-500 rounded-full mt-2"></div>
              </div>

              <div 
                className={`bg-gradient-to-br from-green-500/20 to-green-600/20 backdrop-blur-xl rounded-2xl p-6 border border-green-400/30 shadow-2xl transform transition-all duration-300 cursor-pointer ${activeCard === 'returns' ? 'scale-105 ring-4 ring-green-400' : 'hover:scale-105'}`}
                onMouseEnter={() => setActiveCard('returns')}
                onMouseLeave={() => setActiveCard(null)}
              >
                <div className="flex items-center gap-2 mb-3">
                  <TrendingUp size={20} className="text-green-600" />
                  <p className="text-green-700 text-sm font-semibold">Est. Returns</p>
                </div>
                <p className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{formatCurrency(results.estimatedReturns)}</p>
                <div className="h-1 bg-green-500 rounded-full mt-2"></div>
              </div>

              <div 
                className={`bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-xl rounded-2xl p-6 border border-purple-400/30 shadow-2xl transform transition-all duration-300 cursor-pointer ${activeCard === 'total' ? 'scale-105 ring-4 ring-purple-400' : 'hover:scale-105'}`}
                onMouseEnter={() => setActiveCard('total')}
                onMouseLeave={() => setActiveCard(null)}
              >
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles size={20} className="text-purple-600" />
                  <p className="text-purple-700 text-sm font-semibold">Total Value</p>
                </div>
                <p className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{formatCurrency(results.totalValue)}</p>
            ...
            <div className="bg-white backdrop-blur-xl rounded-2xl p-6 border border-gray-200 shadow-xl">
              ...
              <div className="flex justify-center gap-6 mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700 text-sm">Invested</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="bg-white backdrop-blur-xl rounded-2xl p-6 border border-gray-200 shadow-xl">
              <h3 className="text-gray-900 font-bold text-lg mb-4">Wealth Breakdown</h3>
        ...
        <div className="mt-8 bg-white backdrop-blur-xl rounded-2xl p-8 border border-gray-200 shadow-xl">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-3">Why SIP?</h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                Systematic Investment Plans help you build wealth gradually through disciplined investing. 
                By investing fixed amounts regularly, you benefit from rupee cost averaging and harness 
                the power of compounding - making your money work harder for you over time.
              </p>
              <div className="grid sm:grid-cols-3 gap-4 mt-6">
                <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
                  <p className="text-purple-700 font-semibold mb-1">Disciplined</p>
                  <p className="text-gray-600 text-sm">Regular investing habit</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
                  <p className="text-purple-700 font-semibold mb-1">Flexible</p>
                  <p className="text-gray-600 text-sm">Start with small amounts</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
                  <p className="text-purple-700 font-semibold mb-1">Powerful</p>
                  <p className="text-gray-600 text-sm">Compounding benefits</p>
                </div>
              </div>
            </div>
          </div>
